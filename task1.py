...
def preprocessing(file, case):
    ...
    # case 2 
        ...
        text_rdd_header = text_rdd.first()
        data_rdd = text_rdd.filter(lambda row: row != text_rdd_header)
        rdd = data_rdd.map(lambda row: (row.split(',')[1], row.split(',')[0])).groupByKey().mapValues(set).map(lambda item: sorted(list(item[1])))
    return rdd
...
if __name__ == '__main__':
    ...
    sc.setLogLevel('OFF')
    rdd = preprocessing(input_filepath,case)
